  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground capitalize">{fechaHoy}</p>
          <h1 className="mt-0.5 text-xl font-bold tracking-tight">
            {esGestion ? 'Panel comercial' : `Hola, ${sesion.nombre?.split(' ')[0] ?? 'bienvenido'} 👋`}
          </h1>
          {!esGestion && (
        ...
        <Link href="/comercial/tareas/nueva"
          className="inline-flex items-center gap-1.5 rounded-full bg-primary px-3.5 py-2 text-xs font-semibold text-primary-foreground shadow-sm hover:bg-primary/90 shrink-0">
      ...
      {Object.keys(dash.porEtapa).length > 0 && (
        <section>
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link href="/comercial/proyectos" className="text-xs text-muted-foreground hover:text-foreground">Ver todos →</Link>
          </div>
