...
export default function VehiculosClient({ vehiculos: initVehiculos, tiposCertificado, isAdmin }: Props) {
  ...
  async function handleSave(vehiculoId: string) {
    ...
    if (editingCert) {
      ...
      setVehiculos(prev => prev.map(v =>
        v.id === vehiculoId
          ? { ...v, certificados: v.certificados.map(c => c.id === editingCert ? data : c) }
          : v
      ))
      setEditingCert(null)
    } else {
  ...
  async function handleDelete(vehiculoId: string, certId: string) {
    ...
    setVehiculos(prev => prev.map(v =>
      v.id === vehiculoId ? { ...v, certificados: v.certificados.filter(c => c.id !== certId) } : v
    ))
  }
    ...
    <div className="mb-8">
      ...
      <div className="space-y-3">
        {vehiculos.map(veh => {
          ...
          const worstEstado = veh.certificados.length > 0
          ...
          const estadoColor = {
            vencido: 'bg-red-500', proximo: 'bg-amber-400',
            vigente: 'bg-green-500', sin_fecha: 'bg-gray-300'
                          ...
                          {isEditing ? (
                            ...
                            <div className="flex items-center gap-3 px-4 py-3">
                              <div className="flex-1 min-w-0">
                                ...
                                {cert.notas && <p className="text-xs text-gray-400 mt-0.5 truncate">{cert.notas}</p>}
                              </div>
                              <span className={clsx('inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border shrink-0', ESTADO_COLORS[estado])}>
                          ...
                          {isEditing && (
                            <div className="px-4 pb-4">
                              ...
                              <div className="flex gap-2 mt-3">
                                <button onClick={() => handleSave(veh.id)} disabled={saving || !form.tipo_id} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-xs font-medium px-4 py-2 rounded-lg">
                                  {saving ? 'Guardando...' : 'Guardar'}
                                </button>
                                <button onClick={() => { setEditingCert(null); setForm(FORM_EMPTY) }} className="text-xs text-gray-500 px-3 py-2">Cancelar</button>
                              </div>
                  ...
                  {isAddingHere && isAdmin && (
                    <div className="bg-white rounded-lg border border-indigo-200 p-4">
                      ...
                      <div className="flex gap-2 mt-3">
                        ...
                        <button onClick={() => { setAddingTo(null); setForm(FORM_EMPTY) }} className="text-xs text-gray-500 px-3 py-2">Cancelar</button>
                      </div>
                      {error && <p className="text-xs text-red-500 mt-2">{error}</p>}
                    </div>
                  )}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {form.tipo_id === 'otro' && (
          <div className="col-span-2">
            ...
            <input type="text" value={form.tipo_nombre_custom}
  ...
  function renderForm(_vehiculoId: string, _editing: boolean) { return null }
}
