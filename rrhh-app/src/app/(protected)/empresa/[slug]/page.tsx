...
export default async function EmpresaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: empresa } = await supabase
    .from('empresas').select('*').eq('slug', slug).single()
  if (!empresa) notFound()
  ...
  const [{ data: empleados }, { data: vehiculos }, { data: certsEmpresa }, { data: tiposVehiculo }] = await Promise.all([
    supabase
      .from('empleados')
    ...
    supabase
      .from('vehiculos')
      .select('*, certificados(id, tipo_id, tipo_nombre_custom, fecha_vencimiento, notas, alerta_dias, tipo:tipos_certificado(nombre))')
      .eq('empresa_id', empresa.id).eq('activo', true).order('patente'),
    supabase
      .from('certificados')
      .select('*, tipo:tipos_certificado(nombre)')
      .eq('empresa_id', empresa.id).order('fecha_vencimiento'),
    supabase
      .from('tipos_certificado')
      .select('*').eq('aplica_vehiculo', true).order('orden'),
  ])
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
                  ...
                  {emps.map(emp => {
                    ...
                    return (
                      <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-5 py-3.5"><p className="font-medium text-gray-900">{emp.nombre}</p></td>
      ...
      {/* Vehículos — componente interactivo */}
      <VehiculosClient
        vehiculos={vehiculos ?? []}
