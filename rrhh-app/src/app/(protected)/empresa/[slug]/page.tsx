...
export default async function EmpresaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: empleados } = await supabase
    ...
    .select(`
      *,
      certificados(
        id, fecha_vencimiento, alerta_dias,
  ...
  const { data: certsEmpresa } = await supabase
    .from('certificados')
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">{empleados?.length ?? 0} empleados activos</p>
        </div>
      </div>
                ...
                return (
                  <div key={cert.id} className="flex items-center gap-4 px-5 py-3.5">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-900">{cert.tipo?.nombre ?? cert.tipo_nombre_custom}</p>
                      {cert.numero_documento && <p className="text-xs text-gray-400">{cert.numero_documento}</p>}
                    </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    <th className="text-left px-4 py-3 font-medium text-gray-500 text-xs uppercase tracking-wide">Certificados</th>
                    ...
                    return (
                      <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3.5 text-center">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${ESTADO_COLORS[estado]}`}>
        ...
        <div className="mb-8">
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left px-5 py-3 font-medium text-gray-500 text-xs uppercase tracking-wide">Patente</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-500 text-xs uppercase tracking-wide">VTV</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-500 text-xs uppercase tracking-wide">Seguro</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {vehiculos!.map(veh => {
                  ...
                  return (
                    <tr key={veh.id} className="hover:bg-gray-50">
                      <td className="px-5 py-3.5 font-mono font-medium text-gray-900">{veh.patente}</td>
                      <td className="px-4 py-3.5">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${ESTADO_COLORS[estadoVtv]}`}>
                          {vtv?.fecha_vencimiento ? format(new Date(vtv.fecha_vencimiento), 'dd/MM/yyyy') : '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3.5">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${ESTADO_COLORS[estadoSeg]}`}>
                          {seguro?.fecha_vencimiento ? format(new Date(seguro.fecha_vencimiento), 'dd/MM/yyyy') : '—'}
                        </span>
                      </td>
                    </tr>
                  )
